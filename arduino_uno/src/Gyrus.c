...
void gyrus_run (gyrus_application application) {
  ...
  if (sinks->D13 != NULL) sinks->D13->add_listener (sinks->D13, digital_write_listener_create (GYRUS_PIN_D13));
  if (sinks->LED != NULL) sinks->LED->add_listener (sinks->LED, digital_write_listener_create (GYRUS_PIN_LED));
